...
class Rook(BaseFigure):
    ...
    def _available_moves(self, field):
        ...
        i = 0
        while i <= len(top) - 1:
            if isinstance(field.field[top[i]], field.figures):
                try:
                    del top[i + 1]
                except IndexError:
                    break
        ...
        i = 0
        while i <= len(down) - 1:
            if isinstance(field.field[down[i]], field.figures):
                try:
                    del down[i + 1]
                except IndexError:
                    break
        ...
        i = 0
        while i <= len(left) - 1:
            if isinstance(field.field[left[i]], field.figures):
                try:
                    del left[i + 1]
                except IndexError:
                    break
        ...
        i = 0
        while i <= len(right) - 1:
            if isinstance(field.field[right[i]], field.figures):
                try:
                    del right[i + 1]
                except IndexError:
                    break
